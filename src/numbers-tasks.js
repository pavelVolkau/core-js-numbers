...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(scl / (vec1 * vec2));
}
...
function getLastDigit(value) {
  const val = value % 10;
  return val < 10 ? val : getLastDigit(val);
}
...
/**
 * Returns count of odd numbers from zero to the suming number.
 * The suming number is taken into account.
 *
 * @param {number} number
 * @return {number}
 *
